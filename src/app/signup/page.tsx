...
export default function SignupPage() {
  ...
  // Simplified shader animation for both canvases
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen text-[#1c1b1b] selection:bg-black selection:text-white overflow-x-hidden font-helvetica">
      <style dangerouslySetInnerHTML={{
        __html: `
        .shader-bg {
            background-color: #0F172A; /* Deepened Navy for better mobile contrast */
            position: relative;
            overflow: hidden;
        }
        ...
        @media (max-width: 1023px) {
          .auth-input {
            color: #000000 !important; /* Forces black text when writing on mobile */
          }
          .auth-input:focus {
              /* Change background from white to light gray when active on mobile */
              background: rgba(255, 255, 255, 0.9) !important;
              border-color: #4285F4 !important; /* Accent color border */
              outline: none;
      ...
      <div className="flex flex-col lg:flex-row lg:h-screen lg:overflow-hidden min-h-screen shader-bg lg:bg-transparent">
        ...
        <main className="relative z-10 w-full lg:w-[50%] flex flex-col justify-between items-center p-[24px] md:p-[40px] bg-transparent lg:bg-[#FAF9F5] min-h-screen">
          ...
          <div className="w-full max-w-[420px] my-auto py-12 space-y-[40px] lg:space-y-[32px]">
            <div className="text-left">
              <h1 className="text-[36px] lg:text-[48px] font-bold font-helvetica mb-[8px] text-white lg:text-black tracking-tight">Get Started</h1>
              <p className="text-white/70 lg:text-[#444748] text-[15px] lg:opacity-80">
                Create your account to access the neural network.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-[16px]">
              <div className="space-y-[4px]">
                <label className="block text-[10px] uppercase tracking-[0.1em] font-bold font-['Inter'] text-[#FAF9F5]/70 lg:text-[#8C8880] ml-4 mb-2">Full Name</label>
                <input
            ...
            <p className="text-center text-sm text-white/50 lg:text-[#8C8880]">
              Already have an account? <Link className="text-white lg:text-black font-bold hover:underline transition-all" href="/login">Log in</Link>
            </p>
          </div>
